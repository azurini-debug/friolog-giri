...
# Validità del link operatore per lo scanner cartoni (in giorni).
GIORNI_VALIDITA_SCANNER = 7
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""
                ...
                ALTER TABLE consegne ADD COLUMN IF NOT EXISTS n_cartoni INTEGER;
                ...
                -- Sicurezza: attiva Row-Level Security su tutte le tabelle.
                -- Senza policy, l'API pubblica di Supabase (chiave anon) non vede
                -- nulla; l'app, che si connette in diretto come proprietario del DB,
                -- ignora RLS e continua a funzionare normalmente. Idempotente.
                ALTER TABLE giri            ENABLE ROW LEVEL SECURITY;
                ALTER TABLE consegne        ENABLE ROW LEVEL SECURITY;
                ALTER TABLE articoli        ENABLE ROW LEVEL SECURITY;
                ALTER TABLE articoli_import ENABLE ROW LEVEL SECURITY;
                ALTER TABLE riconoscimenti  ENABLE ROW LEVEL SECURITY;
                ALTER TABLE correzioni      ENABLE ROW LEVEL SECURITY;
                ALTER TABLE scanner_token   ENABLE ROW LEVEL SECURITY;
            """)
            conn.commit()
    finally:
    ...
    for row_el in table.findall("Row"):
        ...
        if len(cells) >= 9 and cells[6]:
            rows.append({
                ...
                "citta":          cells[3],
                "provincia":      cells[4],
                "n_cartoni":      int(cells[5]) if len(cells) > 5 and cells[5].isdigit() else 0,
                    ...
                    cur.execute(
                        """INSERT INTO consegne
                           (giro_id, seq_originale, ragione_soc, indirizzo, citta,
                            provincia, codice_cliente, telefono_cl, note, n_cartoni)
                           VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
                        (giro_id, r["seq_originale"], r["ragione_soc"],
                         r["indirizzo"], r["citta"], r["provincia"],
                         r["codice_cliente"], r["telefono_cl"], r["note"], r["n_cartoni"])
                    )
                db.commit()
            nuovi += 1
...
def giro_autista(token):
    ...
    tot_cartoni = sum((c.get("n_cartoni") or 0) for c in consegne)

    return render_template(
        "giro.html",
        giro=dict(giro),
        consegne=consegne,
        tot_cartoni=tot_cartoni
    )
...
def admin_cartoni():
    ...
    for t in tok_rows:
        ...
        if tel and not tel.startswith("+"):
            tel = "+39" + tel.lstrip("0")
        testo = f"Ciao! Ecco il link per identificare i cartoni (valido {GIORNI_VALIDITA_SCANNER} giorni): {url}"
        t["url"] = url
...
def _scegli_correzione(read_ean, rows):
    """Richiama una correzione SOLO se l'EAN coincide (mai sulla sola descrizione)."""
    if not read_ean:
        return None
    best, best_conf = None, -1
    for r in rows:
        if (r.get("ean") or "") != read_ean:
            continue
        c = int(r.get("conferme") or 1)
        if c > best_conf:
            best, best_conf = r, c
    return best
...
def registra_correzione(codice, descrizione, ean=""):
    """Memorizza una correzione confermata SOLO se c'e' un EAN letto.
    Senza EAN la sola descrizione e' troppo ambigua e non viene salvata."""
    codice = (codice or "").strip()
    ean_n = _solo_cifre(ean)
    if not codice or not ean_n:
        return
    esistente = query(
        "SELECT id FROM correzioni WHERE codice=%s AND ean=%s LIMIT 1",
        (codice, ean_n), fetchone=True,
    )
    if esistente:
        execute(
            "UPDATE correzioni SET conferme=conferme+1, updated_at=NOW() WHERE id=%s",
            (esistente["id"],), commit=True,
        )
    else:
        descr_norm = " ".join(_descr_tokens_set(descrizione))
        execute(
...
def applica_memoria(merged, candidati, confidenza):
    """Se questo EAN e' gia' stato confermato in passato, porta quel codice in cima."""
    ean_n = _solo_cifre(merged.get("ean", ""))
    if not ean_n:
        return candidati, confidenza
    rows = query(
        "SELECT codice, ean, conferme FROM correzioni WHERE ean=%s",
        (ean_n,), fetchall=True,
    ) or []
    scelta = _scegli_correzione(ean_n, [dict(r) for r in rows])
    if not scelta:
        return candidati, confidenza

    cod = scelta["codice"]
    conf_n = int(scelta.get("conferme") or 1)
    motivo = f"confermato via EAN ({conf_n})" if conf_n > 1 else "confermato via EAN"
    ...
    confidenza = max(confidenza, min(98, 90 + conf_n))
    return candidati, confidenza
...
# --- Ufficio: genera un link operatore valido N giorni ---
...
def admin_cartoni_genera_link():
    ...
    token = secrets.token_urlsafe(16)
    execute(
        f"""INSERT INTO scanner_token (token, nome, telefono, scadenza)
           VALUES (%s, %s, %s, NOW() + INTERVAL '{GIORNI_VALIDITA_SCANNER} days')""",
        (token, nome, telefono),
        commit=True,
    )
    flash(f"Link operatore generato: valido {GIORNI_VALIDITA_SCANNER} giorni.", "success")
    return redirect(url_for("admin_cartoni"))


# --- Ufficio: elimina un link operatore ---
@app.route("/admin/cartoni/elimina-link/<int:link_id>", methods=["POST"])
@login_required
def admin_cartoni_elimina_link(link_id):
    execute("DELETE FROM scanner_token WHERE id=%s", (link_id,), commit=True)
    flash("Link eliminato.", "success")
    return redirect(url_for("admin_cartoni"))


# ---------------------------------------------------------------------------
# Cartoni: pagina di controllo dei feedback (accuratezza)
# ---------------------------------------------------------------------------
@app.route("/admin/cartoni/feedback")
@login_required
def admin_cartoni_feedback():
    stats = query(
        """SELECT
             COUNT(*)                                          AS totale,
             COUNT(esito)                                      AS con_feedback,
             COUNT(*) FILTER (WHERE esito = 'corretto')        AS corretti,
             COUNT(*) FILTER (WHERE esito IN ('alternativa','nessuno')) AS sbagliati
           FROM riconoscimenti""",
        fetchone=True,
    )
    righe = query(
        """SELECT id, created_at, n_foto, letto_descrizione, letto_lotto,
                  letto_scadenza, letto_ean, codice_proposto, confidenza,
                  esito, codice_corretto
           FROM riconoscimenti
           ORDER BY id DESC
           LIMIT 200""",
        fetchall=True,
    ) or []
    return render_template("cartoni_feedback.html", stats=stats, righe=righe)
